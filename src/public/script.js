document.addEventListener('DOMContentLoaded', () => {
    ...
    const indexShowMoreContainer = document.getElementById('index-show-more-container');
    ...
    // --- Lógica de Status do Banco de Dados ---
    async function checkDbStatus() {
        try {
            ...
            dbStatusText.textContent = 'Falha na conexão com o servidor/banco de dados.';
        }
    }

    // --- Lógica de Carregamento e Renderização de Tags ---
    async function fetchKeywords() {
        try {
            ...
            if (data.sucesso) {
                ...
                renderTags(indexTagsContainer, indexShowMoreContainer, selectedIndexKeywords, false);
                renderTags(searchTagsContainer, searchShowMoreContainer, selectedSearchKeywords, false);
            } else {
                displayTagError(indexTagsContainer, 'Não foi possível carregar as tags.');
                displayTagError(searchTagsContainer, 'Não foi possível carregar as tags.');
            }
        } catch (error) {
            displayTagError(indexTagsContainer, 'Erro de conexão ao buscar tags.');
            displayTagError(searchTagsContainer, 'Erro de conexão ao buscar tags.');
        }
    }
    ...
    function renderTags(container, showMoreContainer, selectedSet, showAll) {
        ...
        const keywordsToRender = showAll ? allKeywords : allKeywords.slice(0, INITIAL_TAGS_LIMIT);
        ...
        if (!showAll && allKeywords.length > INITIAL_TAGS_LIMIT) {
            ...
            showMoreBtn.textContent = `Mostrar mais ${allKeywords.length - INITIAL_TAGS_LIMIT} tags...`;
            showMoreBtn.onclick = () => renderTags(container, showMoreContainer, selectedSet, true);
        ...
        if (showAll && allKeywords.length > INITIAL_TAGS_LIMIT) {
            ...
            showLessBtn.onclick = () => renderTags(container, showMoreContainer, selectedSet, false);
    ...
    // --- Lógica de Eventos para as Tags ---
    ...
    // --- CORREÇÃO DO BUG: Evento para limpar tags ao clicar no 'X' do input de busca ---
    ...
    indexForm.addEventListener('submit', async (event) => {
        ...
        const inputKeywords = palavrasChaveInput.value.split(',').map(k => k.trim()).filter(Boolean);
        const palavrasChave = Array.from(new Set([...selectedIndexKeywords, ...inputKeywords]));

        if (palavrasChave.length === 0) {
            displayMessage('❌ Erro: Pelo menos uma palavra-chave é necessária.', 'error');
            return;
        }
    ...
    function displayResults(results) {
        ...
        if (results.length === 0) {
            resultsContainer.innerHTML = '<p>Nenhum resultado encontrado para a sua busca.</p>';
            return;
        }
        ...
        results.forEach(site => {
            ...
            const formattedDate = new Date(site.data_cadastro).toLocaleDateString('pt-BR', {
                day: '2-digit', month: '2-digit', year: 'numeric',
                hour: '2-digit', minute: '2-digit'
            });

            // --- NOVA LÓGICA: Criar e exibir as tags do resultado ---
            const tagsHTML = site.palavrasChave && site.palavrasChave.length > 0
                ? `<div class="result-tags-container">
                       ${site.palavrasChave.map(kw => `<span class="result-tag">${kw}</span>`).join('')}
                   </div>`
                : '';
            ...
            resultItem.innerHTML = `
                ...
                <div class="result-meta">
                    <span class="meta-info">Indexado em: ${formattedDate.replace(',', ' às')}</span>
                </div>
            `;
